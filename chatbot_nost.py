...
def respond(input):
    try:
        responses = knowledge_base[input]
        for response in responses:
            print(response)
    except KeyError:
        print(
            "Question is not present in the knowledge base!\nCould you please enter the appropriate answer for the question below-"
        )
        return True  # Indicate that the question was not found
...
while True:
    user_input = input("Enter a query here (type 'quit' to exit): ").lower()
    if user_input == "quit":
